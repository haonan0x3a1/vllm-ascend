...
class AscendSFAMetadata:
    ...
    # CPU block table and computed-token count let Host offload restore a
    # layer's history before processing a later chunked-prefill chunk.
...
class AscendSFAMetadataBuilder(MLACommonMetadataBuilder[AscendSFAMetadata]):
    ...
    def _build(
        ...
        if block_table_cpu is not None:
            block_table_cpu = block_table_cpu[:num_reqs]
        num_computed_tokens_cpu = common_attn_metadata.num_computed_tokens_cpu
        if num_computed_tokens_cpu is not None:
            num_computed_tokens_cpu = num_computed_tokens_cpu[:num_reqs]
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def _restore_sparse_kv_offload_prefill_context(
        ...
        """Restore this layer's Host history into the shared Prefill cache."""
        if (
            not self.sparse_kv_offload_config.enabled
            or self.sparse_kv_offload_config.mode != "host"
            or attn_metadata.attn_state == AscendAttentionState.DecodeOnly
        ):
            return
        ...
        if attn_metadata.num_computed_tokens_cpu is None:
            raise RuntimeError(
                "sparse_kv_offload host mode requires CPU computed-token "
                "metadata for chunked-prefill context restoration."
            )
        if attn_metadata.num_computed_tokens_cpu.numel() != 1:
            raise RuntimeError(
                "sparse_kv_offload host mode currently supports one request, "
                "but received computed-token metadata for "
                f"{attn_metadata.num_computed_tokens_cpu.numel()} requests."
            )

        context_len = int(attn_metadata.num_computed_tokens_cpu[0])
        if context_len == 0:
            return
        workspace = self._get_sparse_kv_offload_workspace(full_kv_cache)
        workspace.restore_prefill_context(
            full_kv_cache,
            attn_metadata.block_table_cpu,
            context_len,
        )
    ...
    def forward(
        ...
        if (
            ...
            # Complete the layerwise load before either restoring a chunked
            # Prefill context or writing new KV. Otherwise restoration can
            # read stale Host data, or a late whole-block load can overwrite
            # rows generated by this forward.
            wait_for_kv_layer_from_connector(layer_name)
            wait_for_layer_load_done = True
        self._restore_sparse_kv_offload_prefill_context(
            full_kv_cache,
            attn_metadata,
        )
